...
void GlobalOptimization::inputGPS(double t, double latitude, double longitude,
  ...
  newGPS = true;
    std::ofstream founB("/roadstar/modules/fusion_localization/results/bestpose.txt",
                        std::ios::app);
    founB.setf(std::ios::fixed, std::ios::floatfield);
    founB.precision(5);
    founB << t << " ";
    founB.precision(5);
    founB << xyz[0] << " "
...
void GlobalOptimization::optimize() {
  while (true) {
    if (newGPS) {
      ...
      int i = 0;
      for (iterLIO = localPoseMap.begin(); iterLIO != localPoseMap.end();
           iterLIO++, i++) {
        // lio factor
        iterLIONext = iterLIO;
        iterLIONext++;
        if (iterLIONext != localPoseMap.end()) {
          ...
          wTi.block<3, 3>(0, 0) =
              Eigen::Quaterniond(iterLIO->second[3], iterLIO->second[4],
                                 iterLIO->second[5], iterLIO->second[6])
                  .toRotationMatrix();
          wTi.block<3, 1>(0, 3) = Eigen::Vector3d(
              iterLIO->second[0], iterLIO->second[1], iterLIO->second[2]);
          wTj.block<3, 3>(0, 0) =
              Eigen::Quaterniond(iterLIONext->second[3], iterLIONext->second[4],
                                 iterLIONext->second[5], iterLIONext->second[6])
                  .toRotationMatrix();
          wTj.block<3, 1>(0, 3) =
              Eigen::Vector3d(iterLIONext->second[0], iterLIONext->second[1],
                              iterLIONext->second[2]);

          Eigen::Matrix4d iTj = wTi.inverse() * wTj;
          Eigen::Quaterniond iQj;
          iQj = iTj.block<3, 3>(0, 0);
          // iQj = Eigen::Quaterniond( iTj.block<3, 3>(0, 0) );
          Eigen::Vector3d iPj = iTj.block<3, 1>(0, 3);

          ceres::CostFunction *lio_function =
              RelativeRTError::Create(iPj.x(), iPj.y(), iPj.z(), iQj.w(),
                                      iQj.x(), iQj.y(), iQj.z(), 0.1, 0.01);
          problem.AddResidualBlock(lio_function, NULL, q_array[i], t_array[i],
...
void GlobalOptimization::updateGlobalPath() {
  ...
  for (iter = globalPoseMap.begin(); iter != globalPoseMap.end(); iter++) {
    ...
    global_path.poses.push_back(pose_stamped);
      ...
      std::ofstream founL("/roadstar/modules/fusion_localization/results/global_fusion.txt",
                          std::ios::app);
      founL.setf(std::ios::fixed, std::ios::floatfield);
      founL.precision(5);
      founL << pose_stamped.header.stamp.toSec() << " ";
      founL.precision(5);
      founL << pose_stamped.pose.position.x << " "
