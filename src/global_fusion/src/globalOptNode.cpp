...
void GPS_callback(const boost::shared_ptr<const roadstar::drivers::gnss::GnssBestPose> GPS_msg)
{
    //printf("GPS_callback! \n");
    double t = GPS_msg->header().timestamp_sec();
    //printf("receive GPS with timestamp %f\n", GPS_msg->header.stamp.toSec());
    double latitude = GPS_msg->latitude();
    double longitude = GPS_msg->longitude();
    double altitude = GPS_msg->height_msl();
    //int numSats = GPS_msg->status.service;
    double pos_accuracy = GPS_msg->undulation() / 5000.0;
...
void lio_callback(const nav_msgs::Odometry::ConstPtr &pose_msg)
{
    ...
    globalEstimator.inputOdom(t, lio_t, lio_q);
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_GPS = n.subscribe("/roadstar/drivers/novatel/best_pose", 100, GPS_callback);
    ros::Subscriber sub_vio = n.subscribe("/laser_odom_to_init", 100, lio_callback);
