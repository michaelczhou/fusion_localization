//
// Created by zhouchang on 19-3-19.
//
...
void laserCloudHandler(const velodyne_msgs::PointCloudConstPtr& laserCloudMsg)
{
    ...
    for (int i = 0; i < cloudSize; i++)
    {
        ...
        {
            LOG_F(WARNING, "wrong scan number");
            ROS_BREAK();
        }
    ...
    cloudSize = count;
    LOG_F(INFO, "points size %d", cloudSize);
...
int main(int argc, char **argv)
{
    ...
    loguru::add_file("regists.log", loguru::Truncate, loguru::Verbosity_MAX);

    ros::init(argc, argv, "lidarRegistration");
    ros::NodeHandle nh;
