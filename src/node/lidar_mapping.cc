...
double para_last_q[4] = {0, 0, 0, 1};
double para_last_p[3] = {0, 0, 0};
...
void process()
{
	while(1)
	{
			...
			!fullResBuf.empty() && !odometryBuf.empty())
		{
			...
			{
				LOG_F(INFO, "time corner %f surf %f full %f odom %f", timeLaserCloudCornerLast, timeLaserCloudSurfLast, timeLaserCloudFullRes, timeLaserOdometry);
			...
			{
				cornerLastBuf.pop();
				LOG_F(INFO, "drop lidar frame in mapping for real time performance");
			}
			...
			LOG_F(INFO, "map corner num %d  surf num %d", laserCloudCornerFromMapNum, laserCloudSurfFromMapNum);
			...
			{
				LOG_F(WARNING, "time Map corner and surf num are not enough");
			}
			transformUpdate();
...
int main(int argc, char **argv)
{
    ...
    loguru::add_file("mapping.log", loguru::Truncate, loguru::Verbosity_MAX);
	...
	LOG_F(INFO, "line resolution %f plane resolution %f", lineRes, planeRes);
